...
-- Added after the table shipped; blogs.companies is derived from this.
alter table public.blogs add column if not exists evidence jsonb not null default '[]'::jsonb;
...
--
-- A run takes minutes, so the request cannot hold the connection. The row is
-- the whole state machine: the worker advances `stage`, the page polls it.
-- ---------------------------------------------------------------------------
create table if not exists public.blog_research_jobs (
  id           uuid primary key default gen_random_uuid(),
...
create index if not exists blog_research_jobs_user_idx   on public.blog_research_jobs (user_id, created_at desc);
